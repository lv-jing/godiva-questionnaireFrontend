...
export default {
  ...
  methods: {
    ...
    addTag(index) {
      if (index === 3) return
      let arr = ['questionThird', 'questionFourth']
      let name = ['tagth1', 'tagth2', 'tagth3']
      let name1 = ['tagfour1', 'tagfour2', 'tagfour3']
      this.list.push({
        name: arr[index - 1],
        list: [
          {
            name: index === 1 ? name[index - 1] : name1[index - 2]
          },
          {
            name: index === 1 ? name[index] : name1[index - 1]
          },
          {
            name: index === 1 ? name[index + 1] : name1[index]
          },
        ]
      })
    },
    removeTags(index, indexs) {
      if (indexs === 1) return
      this.list[index].list.splice(index, 1)
    },
    addTags(index, indexs) {
      if (indexs === 3) return
      console.log(this.list[index].list.push({
        name: 'sasa'
      }));
    },
    async onSubmit() {
